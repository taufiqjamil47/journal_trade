   ...
   if(SendOnTrade)
     {
      if(CheckTradeChanged())
        {
         if(DebugMode)
            Print("Trade change detected, sending data...");
         SendAllTradesData();
        }
     }
   else
   ...
   for(int i = 0; i < total; i++)
     {
      ...
      if(PositionSelectByTicket(ticket))
        {
         string tradeJson = BuildOpenTradeJson(ticket);
         if(tradeJson != "")
           {
            if(count > 0)
               result += ",";
...
//+------------------------------------------------------------------+
//| Build JSON untuk posisi terbuka (LENGKAP)                        |
//+------------------------------------------------------------------+
string BuildOpenTradeJson(ulong ticket)
  {
   string json = "";
   ...
   double stopLoss    = PositionGetDouble(POSITION_SL);    // 🔥 SL TERSEDIA
   double takeProfit  = PositionGetDouble(POSITION_TP);    // 🔥 TP TERSEDIA
   ...
   json += "\"stop_loss\":" + DoubleToString(stopLoss, 5) + ",";    // 🔥 SL
   json += "\"take_profit\":" + DoubleToString(takeProfit, 5) + ","; // 🔥 TP
   ...
   json += "\"is_open\":true";  // 🔥 TANDA POSISI MASIH TERBUKA
   json += "}";
...
//+------------------------------------------------------------------+
//| Build JSON untuk history trade (UPDATED - dengan SL/TP)          |
//+------------------------------------------------------------------+
string BuildHistoryTradeJson(ulong dealTicket)
  {
   ...
   json += "\"entry\":" + DoubleToString(entry, 5) + ",";
   json += "\"stop_loss\":0,";      // 🔥 TIDAK ADA SL DI HISTORY
   json += "\"take_profit\":0,";    // 🔥 TIDAK ADA TP DI HISTORY
   json += "\"exit\":" + DoubleToString(exit, 5) + ",";
   json += "\"timestamp\":\"" + TimeToString(openTime, TIME_DATE|TIME_SECONDS) + "\",";
   json += "\"profit_loss\":" + DoubleToString(profitLoss, 2) + ",";
   json += "\"lot_size\":" + DoubleToString(lotSize, 2) + ",";
   json += "\"mt5_comment\":\"" + comment + "\",";
   json += "\"is_open\":false";    // 🔥 TANDA POSISI SUDAH CLOSE
   json += "}";
